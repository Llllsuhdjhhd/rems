...
1. **角色识别与重要性评定**
   ...
   - **重要性 S/A/B/C/D**：评定角色在**当前碎片文本**内的戏剧权重。
     * S：驱动核心事件的主角，其决策改变情节走向；
     * A：与 S 有直接、关键互动的对象；
     * B：参与事件但非核心互动方；
     * C：在场但无实质参与；
     * D：仅被提及而未出场。
...
2. **【角色快照层级与逻辑（必须严格遵守）】**
   ...
   - **L2：互动逻辑快照**：回答“这个角色在此事件中与他人如何交互？”。必须从 **L3** 中提取与他人的 **具体交互回合与实时反馈**，突显社交关系。
...
3. **【层级分配策略】**
   - **S级角色**：必须同时生成 L1、L2、L3，且必须体现上述的逻辑推导。
   - **A级角色**：必须生成 L1 和 L2。L2 应聚焦其与 S 级角色的交互。L3 留空。
   - **B/C/D级角色**：仅生成 L1。L2、L3 留空。
...
{{
  "roles": [
    {{
      ...
      "snapshot": {{
        "l1_mention": "L1 文本 (S/A/B/C/D 必填)",
        "l2_interaction": "L2 文本 (仅 S/A 级填写；B/C/D 留空)",
        "l3_decision": "L3 文本 (仅 S 级填写)"
      }},
      "emotion": {{
        "anger": 0.0,
